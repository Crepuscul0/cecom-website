...
function translateSecurityDescription(description) {
  const translations = {
...
function generateEnglishContent(item, cleanDescription) {
  const cveMatch = item.title.match(/CVE-\d{4}-\d+/)
  const saMatch = item.title.match(/SA-\d{4}-\d+/)
...
async function importRSSToSupabase(limit = 10) {
  try {
